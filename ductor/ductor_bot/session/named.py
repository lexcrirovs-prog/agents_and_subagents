...
AUTO_FOLLOWUP_SESSION_MAX_AGE_SECONDS = 15 * 60
...
class NamedSessionRegistry:
    ...
    def find_auto_followup_candidate(
        self,
        chat_id: int,
        *,
        prefix: str = "ia-",
        max_age_seconds: float = AUTO_FOLLOWUP_SESSION_MAX_AGE_SECONDS,
    ) -> NamedSession | None:
        """Return a single recent active session suitable for implicit follow-up.

        This is used to smooth UX in direct sub-agent chats: when there is
        exactly one fresh inter-agent session (for example ``ia-main``),
        ordinary user text can continue that session without requiring a
        manual ``@ia-main`` prefix.
        """
        now = time.time()
        candidates = [
            session
            for session in self.list_active(chat_id)
            if session.name.startswith(prefix) and (now - session.created_at) <= max_age_seconds
        ]
        if len(candidates) != 1:
            return None
        return candidates[0]
