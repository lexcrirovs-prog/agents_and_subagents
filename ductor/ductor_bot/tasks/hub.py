...
from ductor_bot.tasks.workstate import render_work_state_handoff, sync_work_state
...
_INTERRUPTED_TASK_ERROR = "Interrupted by shutdown/restart before task completion."
...
Keep your external checkpoint current with: python3 "{checkpoint_helper_path}" ...
"""
...
class TaskHub:
    ...
    def submit(self, submit: TaskSubmit) -> str:
        ...
        checkpoint_helper = self._registry.task_folder(entry.task_id) / "task_checkpoint.py"
        full_prompt = submit.prompt + TASK_PROMPT_SUFFIX.format(
    ...
    def resume(self, task_id: str, follow_up: str, *, parent_agent: str = "") -> str:
        ...
        workstate_path = self._registry.workstate_path(task_id)
        handoff = render_work_state_handoff(workstate_path)
        cold_resume = not entry.session_id
        if cold_resume and not handoff:
            msg = f"Task '{task_id}' has no resumable session or workstate checkpoint"
            raise ValueError(msg)
        ...
        sync_work_state(
            ...
            entry=entry,
            status="running",
            phase="cold-resumed" if cold_resume else "resumed",
            resume_prompt=follow_up,
            clear_questions=previous_status == "waiting",
            result_preview="",
            error="",
        )
        ...
        taskmemory = self._registry.taskmemory_path(entry.task_id)
        checkpoint_helper = self._registry.task_folder(entry.task_id) / "task_checkpoint.py"
        full_prompt = _format_resume_prompt(
            follow_up=follow_up,
            handoff=handoff,
            cold_resume=cold_resume,
            reminder=_RESUME_REMINDER.format(
                taskmemory_path=taskmemory,
                checkpoint_helper_path=checkpoint_helper,
            ),
        )
        self._spawn(
            ...
            full_prompt,
            entry.thinking,
            resume_session=None if cold_resume else entry.session_id,
        )

        logger.info(
            "Task resumed id=%s name='%s' provider=%s cold=%s",
            task_id,
            entry.name,
            entry.provider,
            cold_resume,
    ...
    async def forward_question(self, task_id: str, question: str) -> str:
        ...
        sync_work_state(
            self._registry.workstate_path(task_id),
            entry=entry,
            status="waiting",
            phase="waiting-for-parent",
            question=question,
            next_step="Wait for the parent agent answer, then resume the task.",
        )
    ...
    def active_count(self) -> int:
        """Return the number of delegated tasks still in flight."""
    ...
    async def shutdown(
        ...
        *,
        ...
        if graceful_timeout > 0:
            await self._drain_active(graceful_timeout)
    ...
    async def _drain_active(self, timeout_seconds: float) -> None:
        """Wait briefly for in-flight tasks to finish before forced cancellation."""
        ...
        *,
        ...
        try:
            ...
            self._registry.update_status(
                ...
                elapsed_seconds=elapsed,
                error=_INTERRUPTED_TASK_ERROR,
                result_preview=_INTERRUPTED_TASK_ERROR[:_RESULT_PREVIEW_LEN],
            )
            with contextlib.suppress(Exception):
                await self._deliver(
                    TaskResult(
                        ...
                        result_text=_INTERRUPTED_TASK_ERROR,
                        status="cancelled",
                        elapsed_seconds=elapsed,
    ...
    async def _finalize_result(
        ...
        *,
        ...
        """Persist and deliver the terminal task result without interruption."""
        self._registry.update_status(
            entry.task_id,
            status,
            session_id=session_id,
            ...
            error=error,
            result_preview=result_text[:_RESULT_PREVIEW_LEN],
...
def _format_resume_prompt(
    *,
    ...
    intro = (
        "You are already running inside a fresh background task session. "
        "Use the external checkpoint below to restore state. "
        "Do NOT create or delegate another background task.\n\n"
        if cold_resume
        else "Resume the existing background task. Use the external checkpoint below to restore state.\n\n"
    )
    if not handoff:
