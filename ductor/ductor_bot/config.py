...
def _load_secret_from_runtime_env(ductor_home: str, env_key: str) -> str:
    """Resolve a secret from process env, agent home, then main runtime ``.env``."""
    key = env_key.strip()
    if not key:
        return ""
    ...
    home = Path(ductor_home).expanduser()
    env_files: list[Path] = [home / ".env"]

    # Sub-agents inherit secrets from the main runtime home unless they
    # explicitly override them in their own ``agents/<name>/.env``.
    if home.parent.name == "agents":
        env_files.append(home.parent.parent / ".env")
    ...
    for env_file in env_files:
        if env_file in seen:
            continue
        seen.add(env_file)
        value = load_env_secrets(env_file).get(key, "").strip()
        if value:
            return value
    return ""


def _extract_env_secret_ref(raw: str) -> str:
    """Return env key from ``env:KEY`` references, else empty string."""
    value = raw.strip()
    if not value:
        return ""
    prefix = "env:"
...
class TasksConfig(BaseModel):
    ...
    enabled: bool = True
    max_parallel: int = SAFE_MAX_PARALLEL_TASKS
    timeout_seconds: float = 3600.0

    @field_validator("max_parallel", mode="before")
    @classmethod
    def _normalize_max_parallel(cls, value: object) -> object:
        """Keep delegated-task concurrency in a conservative reliability range."""
        ...
        if parsed < 1:
            logger.warning("tasks.max_parallel=%s is invalid; using 1", value)
            return 1

        if parsed > SAFE_MAX_PARALLEL_TASKS:
            logger.warning(
                "tasks.max_parallel=%s exceeds the conservative safety ceiling; using %d",
                value,
                SAFE_MAX_PARALLEL_TASKS,
            )
            return SAFE_MAX_PARALLEL_TASKS
...
def update_config_file(config_path: Path, **updates: object) -> None:
    ...
    logger.info(
        "Persisted config update: %s",
        ", ".join(
            f"{k}={_sanitize_config_log_value(v, k)}" for k, v in updates.items()
        ),
    )
...
_SECRET_LOG_KEY_MARKERS = ("token", "secret", "password", "api_key")


def _sanitize_config_log_value(value: object, key: str = "") -> object:
    """Redact secrets in config-update log messages while keeping env refs visible."""
    lowered = key.lower()
    if any(marker in lowered for marker in _SECRET_LOG_KEY_MARKERS):
    ...
    if isinstance(value, dict):
        return {k: _sanitize_config_log_value(v, str(k)) for k, v in value.items()}
    if isinstance(value, list):
        return [_sanitize_config_log_value(v) for v in value]
    return value
...
class AgentConfig(BaseModel):
    ...
    def _resolve_secret_refs(self) -> AgentConfig:
        """Resolve ``env:KEY`` references against runtime env secrets."""
        token_env_key = _extract_env_secret_ref(self.telegram_token)
        if token_env_key:
            self.telegram_token = _load_secret_from_runtime_env(self.ductor_home, token_env_key)
        return self
