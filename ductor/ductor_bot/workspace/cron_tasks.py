...
- Do not modify files outside this task folder unless `TASK_DESCRIPTION.md` explicitly tells you to update specific project files as part of the job output.
...
def _create_venv(venv_dir: Path) -> None:
    """Create a Python virtual environment with ``uv`` fallback."""
    try:
        venv.create(venv_dir, with_pip=True)
        return
    except (OSError, subprocess.CalledProcessError):
        logger.warning(
            "stdlib venv creation failed at %s, trying uv fallback",
            venv_dir,
            exc_info=True,
        )
    ...
    if not uv_bin:
        logger.warning("Failed to create .venv at %s (uv not available)", venv_dir)
        return
    ...
    try:
        ...
        logger.warning("Failed to create .venv at %s via uv fallback", venv_dir, exc_info=True)
