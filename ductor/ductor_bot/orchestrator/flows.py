...
def _merge_prompt_layers(*parts: str | None) -> str | None:
    """Join non-empty system-prompt fragments while preserving order."""
...
async def _prepare_normal(
    ...
    *,
    ...
    if is_new:
        ...
        if roster:
            append_prompt = _merge_prompt_layers(append_prompt, roster)
    ...
    hook_prompt = orch._hook_registry.render(hook_ctx)
    prompt = text
    append_prompt = _merge_prompt_layers(append_prompt, hook_prompt)
