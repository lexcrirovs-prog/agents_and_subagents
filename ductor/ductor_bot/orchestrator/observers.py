...
_SHUTDOWN_OBSERVER_GRACE_SECONDS = 20.0
...
class ObserverManager:
    ...
    async def stop_all(self) -> None:
        ...
        if self.background:
            await self.background.shutdown(graceful_timeout=_SHUTDOWN_OBSERVER_GRACE_SECONDS)
        await self.heartbeat.stop()
        if self.webhook:
            await self.webhook.stop()
        if self.cron:
            await self.cron.stop(graceful_timeout=_SHUTDOWN_OBSERVER_GRACE_SECONDS)
