...
class MessageHookRegistry:
    ...
    def collect_suffixes(self, ctx: HookContext) -> list[str]:
        """Return matching hook suffixes for *ctx* without mutating the prompt."""
        suffixes: list[str] = []
    ...
    def render(self, ctx: HookContext) -> str | None:
        """Return the combined hook text for *ctx*, or ``None`` if nothing fired."""
        suffixes = self.collect_suffixes(ctx)
        if not suffixes:
            return None
        return "\n\n".join(suffixes)

    def apply(self, prompt: str, ctx: HookContext) -> str:
        """Backward-compatible prompt appender for callers that still need it."""
...
CONTEXT_BOOTSTRAP = MessageHook(
    name="context_bootstrap",
    condition=on_new_session,
    suffix=(
        "## CONTEXT STATE\n"
        "If `memory_system/context/SESSION_STATE.md`, `OPEN_LOOPS.md`, or "
        "`RECENT_DECISIONS.md` exist, silently review them before long or stateful work.\n"
        "Treat them as the compressed handoff layer between sessions.\n"
        "Keep them compact, factual, and current."
    ),
)
...
DELEGATION_BRIEF = MessageHook(
    ...
    suffix=(
        ...
        "Full docs: tools/task_tools/AGENTS.md."
    ),
)

CONTEXT_HYGIENE_REMINDER = MessageHook(
    name="context_hygiene_reminder",
    condition=every_n_messages(8),
    suffix=(
        "## CONTEXT HYGIENE\n"
        "Silently review `memory_system/context/SESSION_STATE.md`, `OPEN_LOOPS.md`, and "
        "`RECENT_DECISIONS.md` if present.\n"
        "If the session has drifted, large tool outputs accumulated, or a milestone just "
        "completed, compact the live state into those files.\n"
        "Do not dump raw transcript. Save only current focus, open loops, key files, and "
        "durable decisions.\n"
        "If a fresh session would materially help, tell the user plainly and keep the "
        "compact state ready for `/new` or automatic rollover."
    ),
)
...
DELEGATION_REMINDER = MessageHook(
    ...
    suffix=(
        ...
        "instead of creating new ones (keeps context). Docs: tools/task_tools/AGENTS.md."
    ),
)
