...
_SHUTDOWN_INTERRUPT_GRACE_SECONDS = 3.0
...
async def ensure_docker(orch: Orchestrator) -> None:
    ...
    if container:
        orch.update_cli_docker_container(container)
    elif orch._cli_service._config.docker_container:
        logger.warning("Docker recovery failed, falling back to host execution")
        orch.update_cli_docker_container("")


async def _wait_for_active_cli(orch: Orchestrator, timeout_seconds: float) -> int:
    """Wait up to *timeout_seconds* for tracked CLI subprocesses to exit."""
    deadline = asyncio.get_running_loop().time() + max(timeout_seconds, 0.0)
    while True:
        active = orch._process_registry.active_count()
        if active == 0:
            return 0
...
async def _drain_cli_processes(orch: Orchestrator) -> None:
    """Prefer clean CLI exit before falling back to forced shutdown."""
    remaining = await _wait_for_active_cli(orch, _SHUTDOWN_DRAIN_SECONDS)
    if remaining == 0:
        return

    logger.warning(
        "Shutdown drain timed out with %d active CLI process(es); sending SIGINT before force kill",
        remaining,
    )
    interrupted = orch._process_registry.interrupt_all_active()
    if interrupted:
        remaining = await _wait_for_active_cli(orch, _SHUTDOWN_INTERRUPT_GRACE_SECONDS)
        if remaining == 0:
            logger.info(
                "Shutdown drain completed after soft interrupt of %d CLI process(es)",
                interrupted,
            )
            return
    ...
    if killed:
        logger.warning("Shutdown force-killed %d active CLI process(es)", killed)


async def shutdown(orch: Orchestrator) -> None:
    """Cleanup on bot shutdown."""
    await orch._observers.stop_all()
    await _drain_cli_processes(orch)
