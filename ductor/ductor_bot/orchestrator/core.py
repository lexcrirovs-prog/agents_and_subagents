...
class Orchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        self._task_cli_service: CLIService | None = None
        ...
        self._hook_registry.register(CONTEXT_BOOTSTRAP)
        ...
        self._hook_registry.register(CONTEXT_HYGIENE_REMINDER)
    ...
    @property
    def task_cli_service(self) -> CLIService:
        """Dedicated CLI lane for detached background task execution."""
        if self._task_cli_service is None:
            self._task_cli_service = self._cli_service.clone_detached()
        return self._task_cli_service
    ...
    @property
    def cron_observer(self) -> CronObserver | None:
        """Public access to the cron observer."""
        return self._observers.cron

    @property
    def webhook_observer(self) -> WebhookObserver | None:
        """Public access to the webhook observer."""
        return self._observers.webhook
    ...
    def update_cli_default_model(self, model: str) -> None:
        """Keep the primary and detached task CLI lanes on the same default model."""
        self._cli_service.update_default_model(model)
        if self._task_cli_service is not None:
            self._task_cli_service.update_default_model(model)

    def update_cli_reasoning_effort(self, effort: str) -> None:
        """Keep the primary and detached task CLI lanes on the same reasoning default."""
        self._cli_service.update_reasoning_effort(effort)
        if self._task_cli_service is not None:
            self._task_cli_service.update_reasoning_effort(effort)

    def update_cli_docker_container(self, container: str) -> None:
        """Keep the primary and detached task CLI lanes on the same execution target."""
        self._cli_service.update_docker_container(container)
        if self._task_cli_service is not None:
            self._task_cli_service.update_docker_container(container)

    def _build_cli_service_config(self, *, docker_container: str) -> CLIServiceConfig:
        """Build a fresh CLIServiceConfig from the current orchestrator settings."""
        return CLIServiceConfig(
            ...
            docker_container=docker_container,
            claude_cli_parameters=tuple(self._config.cli_parameters.claude),
            codex_cli_parameters=tuple(self._config.cli_parameters.codex),
            gemini_cli_parameters=tuple(self._config.cli_parameters.gemini),
    ...
    async def _route_message(self, dispatch: _MessageDispatch) -> OrchestratorResult:
        ...
        auto_named = None
        if self._agent_name != "main" and dispatch.key.topic_id is None:
            auto_named = self._named_sessions.find_auto_followup_candidate(dispatch.key.chat_id)
        if auto_named is not None:
            session_prompt = directives.cleaned or dispatch.text
            logger.info(
                "Auto-routed direct message into named session name=%s chat=%d",
                auto_named.name,
            ...
            return await named_session_flow(self, dispatch.key, auto_named.name, session_prompt)
    ...
    def _on_config_hot_reload(self, config: AgentConfig, hot: dict[str, object]) -> None:
        ...
        if any(
            ...
            if self._task_cli_service is not None:
                self._task_cli_service.update_config(
                    self._build_cli_service_config(
                        docker_container=self._task_cli_service._config.docker_container
                    )
                )
    ...
    async def inject_prompt(
        ...
        *,
        ...
        """Execute *prompt* in the active session or as an isolated one-shot."""
    ...
    def active_cli_count(self) -> int:
        """Return the number of live CLI subprocesses owned by this orchestrator."""
        return self._process_registry.active_count()
