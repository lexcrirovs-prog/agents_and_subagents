...
def fetch_live_health() -> dict[str, dict[str, object]]:
    ...
    paths = resolve_paths()
    port = _DEFAULT_PORT
    port_hint_path = paths.ductor_home / "internal_api_port.txt"
    if port_hint_path.exists():
        try:
            port = int(port_hint_path.read_text(encoding="utf-8").strip())
        except (OSError, ValueError):
            port = _DEFAULT_PORT
    config_path = paths.config_path
    if port == _DEFAULT_PORT and config_path.exists():
        try:
