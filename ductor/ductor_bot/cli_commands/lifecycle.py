...
_console = Console()
# External stop/restart must wait longer than the runtime's own layered
# shutdown path: supervisor drain, task-hub drain, observer drain, CLI soft
# interrupt grace, and a small buffer for restart-marker pickup.
_GRACEFUL_SHUTDOWN_WAIT_SECONDS = 90.0
_PROCESS_EXIT_POLL_SECONDS = 0.2
...
def _request_graceful_shutdown(pid: int) -> bool:
    if pid <= 0:
        return False
    try:
...
def _graceful_stop_or_fallback(pid: int) -> None:
    from ductor_bot.infra.pidlock import _kill_and_wait

    if _request_graceful_shutdown(pid) and _wait_for_process_exit(
        pid, _GRACEFUL_SHUTDOWN_WAIT_SECONDS
    ):
        return
    _kill_and_wait(pid)


def _request_graceful_restart(pid: int, *, marker_path: Path) -> bool:
    if pid <= 0:
        return False
    write_restart_marker(marker_path=marker_path)
    return _wait_for_process_exit(pid, _GRACEFUL_SHUTDOWN_WAIT_SECONDS)
...
def stop_bot() -> None:
    ...
    if pid_file.exists():
        ...
        if pid is not None and _is_process_alive(pid):
            ...
            _graceful_stop_or_fallback(pid)
...
def cmd_restart() -> None:
    ...
    if pid is not None and _is_process_alive(pid):
        if _request_graceful_restart(pid, marker_path=paths.ductor_home / "restart-requested"):
            return
        _graceful_stop_or_fallback(pid)
        pid_file.unlink(missing_ok=True)
