...
class MessageBus:
    ...
    def unregister_transport(self, transport: TransportAdapter) -> None:
        """Remove a transport adapter if it is currently registered."""
        self._transports = [existing for existing in self._transports if existing is not transport]
        ...
        if envelope.needs_injection and self._injector and envelope.prompt:
            ...
            try:
                response = await self._injector.inject_prompt(
                    ...
                    isolated=envelope.inject_isolated,
                )
