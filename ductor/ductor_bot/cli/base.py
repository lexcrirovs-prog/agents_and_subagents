...
def docker_wrap(
    ...
    *extra_env* vars are injected as ``-e`` flags into ``docker exec``
    ...
    if config.docker_container:
        ...
        container_codex_home = _to_container_path(ductor_home / ".codex", main_home)

        # Merge user secrets from .env (low priority — never override).
        import os

        from ductor_bot.infra.env_secrets import load_runtime_env_secrets

        merged_extra = load_runtime_env_secrets(ductor_home)
        ...
        if config.provider == "codex":
            env_flags += ["-e", f"CODEX_HOME={container_codex_home}"]
        if config.topic_id:
