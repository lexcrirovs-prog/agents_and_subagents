...
class ProcessRegistry:
    ...
    def interrupt_all_active(self) -> int:
        """Send SIGINT to every active process across all chats.

        Unlike :meth:`kill_all_active` this keeps processes registered and gives
        the underlying CLI a chance to unwind cleanly before any hard kill.
        Returns the count of processes signalled.
        """
        total = 0
        for chat_id, entries in self._processes.items():
            interrupted = False
        ...
        if total:
            logger.info("Interrupted %d CLI process(es) across all chats", total)
        return total
    ...
    def active_count(self) -> int:
        """Return the total number of live subprocesses across all chats."""
        return sum(
            1
