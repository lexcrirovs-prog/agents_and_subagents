...
class CodexCLI(BaseCLI):
    ...
    def _compose_prompt(self, prompt: str) -> str:
        """Return only the end-user prompt text.

        Codex already injects AGENTS.md automatically. Additional dynamic
        Ductor guidance should travel via ``developer_instructions`` config
        instead of being mixed into the user-role prompt.
        """
        return prompt

    def _developer_instructions(self) -> str | None:
        """Build dynamic internal instructions for Codex via config override."""
        parts: list[str] = []
    ...
    def _config_override(key: str, value: str) -> str:
        """Encode a config override as a TOML-safe ``key=value`` pair."""
    ...
    def _build_resume_command(
        ...
        if dev_instructions:
            cmd += ["-c", self._config_override("developer_instructions", dev_instructions)]
