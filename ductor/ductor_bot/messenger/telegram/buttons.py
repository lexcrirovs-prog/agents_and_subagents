...
Optional explicit callback data is supported via
``[button:Label|callback_data]``.  When omitted, the label itself becomes the
callback payload (the historical behavior).
...
def _parse_button_payload(payload: str) -> tuple[str, str] | None:
    """Split ``Label|callback`` payload into display label and callback data."""
    label, sep, callback = payload.partition("|")
    label = label.strip()
    if not label:
        return None
    callback_data = callback.strip() if sep else label
    if not callback_data:
        callback_data = label
    return label, _truncate_callback_data(callback_data)
...
def extract_buttons(text: str) -> tuple[str, InlineKeyboardMarkup | None]:
    ...
    def _process_line(line: str) -> str:
        ...
        for m in matches:
            parsed = _parse_button_payload(m.group(1))
            if parsed is None:
                continue
            label, callback_data = parsed
            btns.append(
