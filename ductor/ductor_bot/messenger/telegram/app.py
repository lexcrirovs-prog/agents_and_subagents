...
class TelegramBot:
    ...
    def __init__(
        ...
        *,
        ...
        self._restart_waiting_for_idle = False
        self._restart_last_blockers: int | None = None
        ...
        self._backup_refresh_lock = asyncio.Lock()
        ...
        self._transport_adapter = TelegramTransport(self)
        self._bus.register_transport(self._transport_adapter)
        ...
        self._sequential.set_restart_pending_checker(self._restart_marker_pending)
    ...
    def set_restart_idle_checker(self, callback: Callable[[], int]) -> None:
        """Set a callback that reports runtime-wide restart blockers."""
    ...
    def _restart_marker_pending(self) -> bool:
        orch = self._orchestrator
        if orch is None:
            return False
        return (orch.paths.ductor_home / "restart-requested").exists()

    def _clear_restart_wait_state(self) -> None:
        self._restart_waiting_for_idle = False
        self._restart_last_blockers = None

    def _restart_blocker_count(self) -> int:
        callback = self._restart_idle_checker
        if callback is None:
            return 0
        try:
            return max(int(callback()), 0)
        except Exception:
            logger.exception("Restart idle checker failed")
            return 1

    def _note_restart_deferred(self, blockers: int) -> None:
        if self._restart_waiting_for_idle and self._restart_last_blockers == blockers:
            return
        logger.info(
            "Restart requested but deferred: waiting for %d blocker(s) to drain",
            blockers,
        )
        self._restart_waiting_for_idle = True
        self._restart_last_blockers = blockers

    async def _attempt_restart_when_idle(
        self,
        *,
        marker: Path,
        consume_marker: bool,
    ) -> bool:
        blockers = self._restart_blocker_count()
        if blockers > 0:
            self._note_restart_deferred(blockers)
            return False
        if consume_marker and not await asyncio.to_thread(consume_restart_marker, marker_path=marker):
            self._clear_restart_wait_state()
            return False
        self._clear_restart_wait_state()
        logger.info("Restart request accepted: runtime is idle, stopping polling")
    ...
    async def _on_restart(self, message: Message) -> None:
        if self._config.group_mention_only and not self._is_addressed(message):
            return
        from ductor_bot.infra.restart import write_restart_marker, write_restart_sentinel
        ...
        await self._attempt_restart_when_idle(marker=marker, consume_marker=True)
    ...
    async def _on_callback_query(self, callback: CallbackQuery) -> None:
        ...
        if self._restart_marker_pending():
            await send_rich(
                self._bot,
                chat_id,
                t("startup.restart_body"),
                SendRichOpts(reply_to_message_id=msg.message_id, thread_id=thread_id),
            )
            return
    ...
    async def _resolve_text(self, message: Message) -> str | None:
        ...
        if has_media(message):
            ...
            return await resolve_media_text(
                ...
                paths.workspace,
                preferred_transcript_language=self._config.language,
            )
        if not message.text:
            return None
        if is_group:
        ...
        try:
            while True:
                ...
                if not marker.exists():
                    self._clear_restart_wait_state()
                    continue
                if await self._attempt_restart_when_idle(marker=marker, consume_marker=True):
                    break
    ...
    async def shutdown(self) -> None:
        ...
        if self._orchestrator and self._owns_orchestrator:
