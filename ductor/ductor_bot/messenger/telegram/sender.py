...
_SEND_RETRY_ATTEMPTS = 2
_RETRY_AFTER_PADDING_SECONDS = 0.25
_MAX_TEXT_CHUNKS = 8
_TRUNCATION_NOTICE = (
    "[Output truncated after 8 Telegram messages to avoid flooding. "
    "Ask for a shorter answer or a file if you need the rest.]"
)
...
async def _send_message_with_retry(
    ...
    *,
    ...
    """Send a Telegram message and retry once or twice on rate-limit."""
    kwargs = {
        "chat_id": chat_id,
        "text": text,
    ...
    while True:
        try:
            ...
            if attempt > _SEND_RETRY_ATTEMPTS:
                raise
            delay = max(float(getattr(exc, "retry_after", 0.0) or 0.0), 0.0)
            delay += _RETRY_AFTER_PADDING_SECONDS
            logger.warning(
                "Telegram rate-limited send; retrying in %.2fs (attempt %d/%d)",
                delay,
                attempt,
...
def _cap_text_chunks(chunks: Sequence[str]) -> list[str]:
    """Bound total Telegram fan-out for one logical text response."""
    filtered = [chunk for chunk in chunks if chunk.strip()]
    if len(filtered) <= _MAX_TEXT_CHUNKS:
        return filtered

    logger.warning(
        "Truncating oversized Telegram send from %d chunks to %d",
        len(filtered),
        _MAX_TEXT_CHUNKS,
    )
    return filtered[: _MAX_TEXT_CHUNKS - 1] + [_TRUNCATION_NOTICE[:TELEGRAM_MSG_LIMIT]]


def _plain_fallback_chunks(chunks: Sequence[str]) -> list[str]:
    """Convert failed HTML chunks into a bounded plain-text fallback."""
    remaining = "\n\n".join(chunks)
    plain = html_mod.unescape(re.sub(r"<[^>]+>", "", remaining))
    plain_chunks = split_html_message(plain)
    return _cap_text_chunks(plain_chunks)
...
async def _send_text_chunks(
    ...
    *,
    ...
    chunks = _cap_text_chunks(split_html_message(html_text))
    for i, chunk in enumerate(chunks):
        try:
            last_msg = await _send_message_with_retry(
                bot,
                chat_id=chat_id,
                text=chunk,
            ...
            for pc in _plain_fallback_chunks(chunks[i:]):
                if not pc.strip():
                    continue
                last_msg = await _send_message_with_retry(
                    bot,
