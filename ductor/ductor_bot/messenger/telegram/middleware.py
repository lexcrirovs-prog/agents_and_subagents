...
RestartPendingChecker = Callable[[], bool]
"""Sync callback reporting whether restart is pending and new work is frozen."""
...
_SAFE_DURING_RESTART_COMMANDS: frozenset[str] = frozenset(
    {
        "/restart",
        "/help",
        "/start",
...
def is_safe_during_restart(text: str, bot_username: str | None = None) -> bool:
    """Return True for commands that remain safe while restart is pending."""
    cmd_part = text.strip().lower().split(None, 1)[0] if text.strip() else ""
    if "@" in cmd_part:
        cmd, mention = cmd_part.split("@", 1)
        if bot_username and mention != bot_username.lower():
            return False
        return cmd in _SAFE_DURING_RESTART_COMMANDS
    return cmd_part in _SAFE_DURING_RESTART_COMMANDS
...
class SequentialMiddleware(BaseMiddleware):
    ...
    def __init__(
        ...
        self._restart_pending_checker: RestartPendingChecker | None = None
    ...
    def set_restart_pending_checker(self, checker: RestartPendingChecker) -> None:
        """Register a callback that freezes new work while restart is pending."""
        self._restart_pending_checker = checker
    ...
    async def _notify_restart_pending(self, event: Message) -> None:
        if not self._bot:
            return
        try:
            ...
            logger.debug("Failed to send restart-pending notice", exc_info=True)
    ...
    async def __call__(
        ...
        restart_pending = (
            self._restart_pending_checker is not None and self._restart_pending_checker()
        )
        if restart_pending and not (
            text
            and (
                is_quick_command(text, self._bot_username)
                or is_safe_during_restart(text, self._bot_username)
            )
        ):
            logger.info("Restart pending: rejected new message chat=%d", chat_id)
            await self._notify_restart_pending(event)
            return None
