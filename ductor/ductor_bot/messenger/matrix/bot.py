...
class MatrixBot:
    ...
    def __init__(
        ...
        *,
        ...
        self._transport_adapter = MatrixTransport(self)
    ...
    def _restart_pending(self) -> bool:
        return _expand_marker(self._config.ductor_home).exists()
    ...
    async def _attempt_restart_when_idle(
        ...
        *,
        ...
        if consume_marker and not await asyncio.to_thread(consume_restart_marker, marker_path=marker):
            ...
            return False
        self._clear_restart_wait_state()
        logger.info("Restart request accepted: runtime is idle, stopping Matrix sync")
        self._exit_code = EXIT_RESTART
        if self._sync_task and not self._sync_task.done():
            self._sync_task.cancel()
        return True

    async def _send_restart_pending_notice(self, room_id: str) -> None:
        await self._send_rich(room_id, t("startup.restart_body"))
    ...
    async def _cmd_restart(
        ...
        await self._attempt_restart_when_idle(marker=marker, consume_marker=False)
    ...
    async def _dispatch_with_lock(
        ...
        if self._restart_pending():
            await self._send_restart_pending_notice(room_id)
            return
    ...
    async def _run_handler_with_lock(
        ...
        room_id: str = kwargs["room_id"]  # type: ignore[assignment]
        if self._restart_pending():
            await self._send_restart_pending_notice(room_id)
            return
        lock = self._lock_pool.get(key.lock_key)
    ...
    async def _watch_restart_marker(self) -> None:
        ...
        while True:
            ...
            if not marker.exists():
                self._clear_restart_wait_state()
                continue
            if await self._attempt_restart_when_idle(marker=marker, consume_marker=False):
                break
