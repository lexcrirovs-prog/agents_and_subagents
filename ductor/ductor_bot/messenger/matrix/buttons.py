...
class ButtonTracker:
    ...
    def extract_and_format(self, room_id: str, text: str) -> str:
        ...
        ``[button:Label|callback_data]`` is also supported. When callback data
        is omitted, the label itself is used.
        """
        payloads: list[str] = _BUTTON_RE.findall(text)
        if not payloads:
            return text
        ...
        for payload in payloads:
            ...
            if not label:
                continue
            labels.append(label)
            callback_data.append(callback.strip() if sep and callback.strip() else label)
        if not labels:
            return cleaned
