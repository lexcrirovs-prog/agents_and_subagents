...
try:  # Optional dependency: Matrix transport can be disabled in some runtimes.
    from nio import DownloadError as _NioDownloadError
except ImportError:  # pragma: no cover - exercised in environments without nio
    _NioDownloadError = None
...
def _is_download_error(response: object) -> bool:
    """Return ``True`` when *response* is a matrix-nio download error object."""
    if _NioDownloadError is not None and isinstance(response, _NioDownloadError):
        return True
    return response.__class__.__name__.endswith("DownloadError")
