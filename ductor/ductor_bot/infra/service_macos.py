...
def _sanitize_plist_text(value: str) -> str:
    """Normalize surrogate-escaped filesystem text before plist XML encoding."""
...
def _sanitize_plist_value(value: Any) -> Any:
    if isinstance(value, str):
        return _sanitize_plist_text(value)
    if isinstance(value, list):
        return [_sanitize_plist_value(item) for item in value]
    if isinstance(value, dict):
        return {
            _sanitize_plist_value(key) if isinstance(key, str) else key: _sanitize_plist_value(item)
...
def _generate_plist_data(binary_path: str) -> dict[str, Any]:
    ...
    plist_data = {
        ...
        "EnvironmentVariables": {
            "PATH": ":".join(path_dirs),
            "HOME": str(home),
            "DUCTOR_HOME": str(paths.ductor_home),
            "DUCTOR_FRAMEWORK_ROOT": str(paths.framework_root),
            "DUCTOR_SUPERVISOR": "1",
        },
        "StandardOutPath": str(paths.logs_dir / "service.log"),
        "StandardErrorPath": str(paths.logs_dir / "service.err"),
    }
    return _sanitize_plist_value(plist_data)
