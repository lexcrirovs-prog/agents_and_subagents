...
def load_runtime_env_secrets(ductor_home: Path) -> dict[str, str]:
    """Load merged runtime secrets for an agent home.

    Precedence inside the merged mapping is:

    1. main runtime ``.env`` (fallback for sub-agents)
    2. current agent ``.env`` (override for sub-agents)

    Host environment variables are intentionally not considered here; callers
    apply those separately so real process env always stays highest priority.
    """
    ...
    for env_file in env_files:
        ...
        if resolved in seen:
            continue
        seen.add(resolved)
        merged.update(load_env_secrets(env_file))
    return merged
