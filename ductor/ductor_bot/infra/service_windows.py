...
def _escape_python_literal(value: str) -> str:
    """Escape a string for a single-quoted Python literal."""
    return value.replace("\\", "\\\\").replace("'", "\\'")


def _build_pythonw_arguments() -> str:
    """Build pythonw arguments that inject runtime env before starting ductor."""
    paths = resolve_paths()
    code = "; ".join(
        [
            "import os, runpy",
            (
                ...
                f"'DUCTOR_HOME', '{_escape_python_literal(str(paths.ductor_home))}')"
            ),
            (
                "os.environ.setdefault("
                "'DUCTOR_FRAMEWORK_ROOT', "
                f"'{_escape_python_literal(str(paths.framework_root))}')"
            ),
            "os.environ.setdefault('DUCTOR_SUPERVISOR', '1')",
            "runpy.run_module('ductor_bot', run_name='__main__')",
        ]
    )
    return f'-c "{code}"'
...
def _escape_cmd_value(value: str) -> str:
    """Escape a value for Windows cmd.exe set syntax."""
    return value.replace('"', '""')


def _build_binary_wrapper_arguments(binary: str) -> str:
    """Build cmd.exe arguments that inject runtime env before launching the binary."""
    paths = resolve_paths()
    ductor_home = _escape_cmd_value(str(paths.ductor_home))
    framework_root = _escape_cmd_value(str(paths.framework_root))
    binary_cmd = _escape_cmd_value(binary)
    return (
        '/d /s /c ""'
        f'set "DUCTOR_HOME={ductor_home}" && '
        f'set "DUCTOR_FRAMEWORK_ROOT={framework_root}" && '
        'set "DUCTOR_SUPERVISOR=1" && '
        f'"{binary_cmd}"'
        '""'
    )
...
def install_service(console: Console | None = None) -> bool:
    ...
    if pythonw:
        command = pythonw
        arguments = _build_pythonw_arguments()
    else:
        ...
        if not binary:
            ...
            return False
        command = "cmd.exe"
        arguments = _build_binary_wrapper_arguments(binary)
