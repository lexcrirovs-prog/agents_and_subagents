...
def _quote_environment(name: str, value: str) -> str:
    """Render a systemd Environment line with minimal escaping."""
...
def _generate_service_unit(binary_path: str) -> str:
    ...
    env_lines = "\n".join(
        [
            _quote_environment("PATH", path_value),
            _quote_environment("HOME", str(home)),
            _quote_environment("DUCTOR_HOME", str(paths.ductor_home)),
            _quote_environment("DUCTOR_FRAMEWORK_ROOT", str(paths.framework_root)),
            _quote_environment("DUCTOR_SUPERVISOR", "1"),
        ]
    )
