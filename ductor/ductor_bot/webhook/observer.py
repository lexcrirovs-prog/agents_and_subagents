...
class WebhookObserver(BaseTaskObserver):
    ...
    async def _dispatch(self, hook_id: str, payload: dict[str, Any]) -> WebhookResult:
        ...
        if self.restart_pending():
            logger.info("Webhook dispatch skipped: restart pending hook=%s", hook_id)
            return WebhookResult(
                hook_id=hook_id,
                hook_title=hook.title,
                mode=hook.mode,
                result_text="",
                status="skipped:restart_pending",
            )
        ...
        try:
            ...
            self._active_dispatches = max(self._active_dispatches - 1, 0)
