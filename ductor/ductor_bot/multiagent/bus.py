...
_INTERRUPTED_ASYNC_ERROR = "Interrupted by shutdown/restart before completion."
...
class InterAgentBus:
    ...
    def _display_agent_name(self, agent_name: str) -> str:
        """Return a user-facing label for an agent in notifications."""
        if agent_name == "main":
            return t("multiagent.main_agent_name")
        stack = self._agents.get(agent_name)
        configured_name = getattr(getattr(stack, "config", None), "public_name", "")
    ...
    def active_async_count(self) -> int:
        """Return the number of async inter-agent tasks still running."""
        return sum(
            1
    ...
    def _mark_async_terminal(self, task: AsyncInterAgentTask) -> None:
        """Stop counting *task* as in-flight once recipient execution is done.

        Result delivery back into the sender chat may still be pending behind a
        per-chat lock. That delivery should not block restart/shutdown drain or
        be treated as unfinished recipient work.
        """
    ...
    async def _run_async(self, task: AsyncInterAgentTask) -> None:
        ...
        try:
            ...
            orch = target.bot.orchestrator
            if orch is None:
                self._mark_async_terminal(task)
                await asyncio.shield(
                    self._deliver_async_result(
            ...
            # User-facing lifecycle follows the chat that owns visible work.
            # Direct sub-agent chats already show the agent's real reply, so
            # upward summaries back to ``main`` stay silent.
            if self._should_emit_sender_status(task):
                await self._notify_sender_status(task, completed=False)
            ...
            self._mark_async_terminal(task)
            if self._should_emit_sender_status(task):
                await self._notify_sender_status(task, completed=True, session_name=session_name)
            await asyncio.shield(
                self._deliver_async_result(
                    AsyncInterAgentResult(
                        task_id=task.task_id,
    ...
    def _should_emit_sender_status(self, task: AsyncInterAgentTask) -> bool:
        """Return whether user-facing lifecycle statuses should be shown.

        Statuses are useful when the user is waiting on hidden delegated work:
        ``main -> specialist`` and peer consults like ``agent A -> agent B``.
        They are noise for upward operational summaries like ``agent A -> main``
        because the user already sees the direct reply from agent A.
        """
        return task.sender == "main" or task.recipient != "main"
    ...
    async def _notify_recipient(self, task: AsyncInterAgentTask) -> None:
        ...
        try:
            ...
            sender_label = self._display_agent_name(task.sender)
            text = t(
                "multiagent.async_task_accepted_recipient",
                sender=sender_label,
    ...
    async def _notify_sender_status(
        self,
        task: AsyncInterAgentTask,
        *,
        completed: bool,
        ...
        """Notify the sender's user(s) that the delegated task was accepted or completed."""
        try:
            sender = self._agents.get(task.sender)
            if sender is None:
                return

            ns = sender.bot.notification_service
            session = session_name or f"ia-{task.sender}"
            key = (
                "multiagent.async_task_completed_sender"
                if completed
                else "multiagent.async_task_accepted_sender"
            )
            sender_label = self._display_agent_name(task.sender)
            recipient_label = self._display_agent_name(task.recipient)
            text = t(
                key,
                sender=sender_label,
            ...
            if task.chat_id:
                await ns.notify(task.chat_id, text)
            else:
                await ns.notify_all(text)
        except Exception:
            logger.debug(
                "Failed to notify sender '%s' about async task %s status=%s (non-critical)",
                task.sender,
                task.task_id,
    ...
    async def _notify_recipient_completed(
        self,
        task: AsyncInterAgentTask,
        *,
        session_name: str = "",
    ) -> None:
        """Notify the recipient's user(s) that the delegated task has been completed."""
        try:
            target = self._agents.get(task.recipient)
            if target is None:
                return
            ...
            recipient_label = self._display_agent_name(task.recipient)
            text = t(
                "multiagent.async_task_completed_recipient",
                sender=sender_label,
                recipient=recipient_label,
                session=session,
                task_id=task.task_id,
            ...
            logger.debug(
                "Failed to notify recipient '%s' about async task %s completion (non-critical)",
                task.recipient,
                task.task_id,
                exc_info=True,
            )
    ...
    async def cancel_all_async(self, *, reason: str = _INTERRUPTED_ASYNC_ERROR) -> int:
                ...
                await asyncio.shield(
                    self._deliver_async_result(
                        AsyncInterAgentResult(
                            ...
                            success=False,
                            error=reason,
                            elapsed_seconds=time.time() - task.timestamp,
                            original_message=task.message,
                            chat_id=task.chat_id,
                            topic_id=task.topic_id,
                        )
