...
class AgentSupervisor:
    ...
    async def start(self) -> int:
        ...
        self._internal_api = InternalAgentAPI(
            ...
            port_file=self._main_paths.ductor_home / "internal_api_port.txt",
        )
        self._internal_api.set_health_ref(self._health)
        self._internal_api.set_status_provider(self._runtime_status)
        started = await self._internal_api.start()
        if not started:
            msg = "Internal agent API failed to start"
            raise RuntimeError(msg)
        if self._main_config.interagent_port != self._internal_api.port:
            logger.warning(
                "Configured interagent_port=%d was unavailable, runtime switched to %d",
                self._main_config.interagent_port,
                self._internal_api.port,
            )
            self._main_config.interagent_port = self._internal_api.port
    ...
    def _inject_supervisor_hook(self, stack: AgentStack) -> None:
        ...
        async def _post_startup() -> None:
            ...
            stack.bot.set_restart_idle_checker(supervisor.restart_blocker_count)
            health = supervisor._health.get(stack.name)
            if stack.is_main:
    ...
    def _runtime_status(self) -> dict[str, object]:
        """Build runtime-wide readiness metadata for the internal API."""
        registered_agents = self._bus.list_agents() if self._bus is not None else []
        return {
            "running": self._running,
            "main_ready": self._main_ready.is_set(),
            "task_hub_enabled": self._task_hub is not None,
    ...
    def _wire_task_hub(self, stack: AgentStack) -> None:
        ...
        # Main keeps ownership/result routing, but tasks execute on a detached
        # CLI lane so delegated work does not occupy the interactive main lane.
        task_cli = orch.task_cli_service if name == "main" else orch.cli_service
        hub.set_cli_service(name, task_cli)
    ...
    async def _start_sub_agent(self, sub_cfg: SubAgentConfig) -> None:
        ...
        # Sync critical runtime-facing fields so local tools inside the sub-agent
        # workspace read the same effective config the runtime is using.
        config_path = agent_home / "config" / "config.json"
        if config_path.exists():
            await update_config_file_async(
                config_path,
                public_name=config.public_name,
                ductor_home=str(agent_home),
                ...
                telegram_token=sub_cfg.telegram_token,
                allowed_user_ids=config.allowed_user_ids,
                allowed_group_ids=config.allowed_group_ids,
                streaming=config.streaming.model_dump(),
                docker=config.docker.model_dump(),
                heartbeat=config.heartbeat.model_dump(),
                cleanup=config.cleanup.model_dump(),
                webhooks=config.webhooks.model_dump(),
                api=config.api.model_dump(),
                cli_parameters=config.cli_parameters.model_dump(),
                image=config.image.model_dump(),
                scene=config.scene.model_dump(),
                timeouts=config.timeouts.model_dump(),
    ...
    def _active_cli_count(self) -> int:
        """Count live CLI subprocesses across all running agents."""
        total = 0
        for stack in self._stacks.values():
            orch = stack.bot.orchestrator
            if orch is None:
                continue
            total += orch.active_cli_count()
        return total

    def _active_async_count(self) -> int:
        """Count live async inter-agent tasks."""
        if self._bus is None:
            return 0
        return self._bus.active_async_count()

    def _active_background_count(self) -> int:
        """Count in-flight background observer tasks across all agents."""
        total = 0
        for stack in self._stacks.values():
            orch = stack.bot.orchestrator
            if orch is None or orch.bg_observer is None:
                continue
            total += orch.bg_observer.active_count()
        return total

    def _active_cron_count(self) -> int:
        """Count cron jobs currently executing across all agents."""
        total = 0
        for stack in self._stacks.values():
            orch = stack.bot.orchestrator
            if orch is None or orch.cron_observer is None:
                continue
            total += orch.cron_observer.active_count()
        return total

    def _active_webhook_count(self) -> int:
        """Count webhook dispatches currently executing across all agents."""
        total = 0
        for stack in self._stacks.values():
            orch = stack.bot.orchestrator
            if orch is None or orch.webhook_observer is None:
                continue
            total += orch.webhook_observer.active_count()
        return total

    def _active_task_hub_count(self) -> int:
        """Count detached background-task lane work tracked by TaskHub."""
        if self._task_hub is None:
            return 0
        return self._task_hub.active_count()

    def restart_blocker_count(self) -> int:
        """Count runtime-wide work that should defer a marker-triggered restart."""
        return (
            self._active_cli_count()
            + self._active_async_count()
            + self._active_background_count()
            + self._active_cron_count()
            + self._active_webhook_count()
            + self._active_task_hub_count()
        )

    async def _drain_inflight_work(self, timeout_seconds: float = _SHUTDOWN_DRAIN_SECONDS) -> None:
        """Give active CLI and async inter-agent work a chance to finish before shutdown."""
        deadline = asyncio.get_running_loop().time() + max(timeout_seconds, 0.0)
        while True:
            active_cli = self._active_cli_count()
            active_async = self._active_async_count()
            if active_cli == 0 and active_async == 0:
                return
            if asyncio.get_running_loop().time() >= deadline:
                logger.warning(
                    "Shutdown drain timed out with %d active CLI process(es) and %d async inter-agent task(s)",
                    active_cli,
                    active_async,
                )
                return
    ...
    async def stop_all(self) -> None:
        ...
        # Stop task hub while bots are still alive so interruption notices/results
        # can still be delivered back into the parent chat.
        if self._task_hub:
            await self._task_hub.shutdown(graceful_timeout=_SHUTDOWN_DRAIN_SECONDS)
