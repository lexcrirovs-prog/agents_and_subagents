...
_PORT_FILE_NAME = "internal_api_port.txt"
class InternalAgentAPI:
    ...
    def __init__(
        ...
        *,
        ...
        self._status_provider: Callable[[], dict[str, object]] | None = None
    ...
    def set_status_provider(self, provider: Callable[[], dict[str, object]]) -> None:
        """Set a callback that returns runtime-wide readiness metadata."""
    ...
    async def start(self) -> bool:
        ...
        try:
            self._port = await self._start_site(requested_port)
        except OSError:
            fallback_port = None if requested_port == 0 else self._pick_fallback_port()
            ...
            logger.warning(
                "Internal agent API port %d is busy, retrying on free port %d",
                requested_port,
                fallback_port,
                exc_info=True,
            )
            try:
                self._port = await self._start_site(fallback_port)
            except OSError:
                logger.exception(
                    "Failed to start internal agent API on fallback port %d",
                    fallback_port,
                )
                await self._cleanup_failed_start()
                return False
    ...
    async def _cleanup_failed_start(self) -> None:
        """Best-effort cleanup after a failed bind/start attempt."""
        self._clear_port_file()
    ...
    async def _start_site(self, port: int) -> int:
        """Bind and start the aiohttp site, returning the actual listening port."""
        assert self._runner is not None
        site = web.TCPSite(self._runner, self._bind_host, port)
        await site.start()
        self._site = site
        if port != 0:
            return port
    ...
    def _pick_fallback_port(self) -> int | None:
        """Reserve a free localhost port for one-shot fallback binding."""
        probe_host = "127.0.0.1" if self._bind_host == _BIND_ALL_HOST else self._bind_host
        try:
    ...
    def _write_port_file(self) -> None:
        """Persist the currently selected port for helper CLI commands."""
        if self._port_file is None:
            return
        try:
            self._port_file.parent.mkdir(parents=True, exist_ok=True)
            self._port_file.write_text(f"{self._port}\n", encoding="utf-8")
        except OSError:
            logger.warning("Failed to persist internal API port file", exc_info=True)

    def _clear_port_file(self) -> None:
        """Remove the runtime port hint file if it exists."""
        if self._port_file is None:
            return
        try:
            self._port_file.unlink(missing_ok=True)
        except OSError:
            logger.warning("Failed to remove internal API port file", exc_info=True)
    ...
    async def _handle_health(self, request: web.Request) -> web.Response:
        ...
        if self._status_provider is not None:
            try:
                ...
                logger.warning("Failed to build internal API runtime status", exc_info=True)
