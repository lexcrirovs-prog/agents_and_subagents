...
This is the top-level Ductor home directory for the current runtime.
In classic installs it is `~/.ductor`; in repo-local deployments it is often
`runtime/ductor-home`.
...
4. `workspace/shared/00-HOME.md` if present (canonical shared cross-agent knowledge)
5. `config/CLAUDE.md` (only for config changes)
...
- `agents.json` in this Ductor home is the single source of truth for all sub-agent
...
`SHAREDMEMORY.md` in this Ductor home contains facts shared across all agents
...
- `shared/` under this Ductor home is the canonical shared document layer for richer,
  structured cross-agent knowledge: user profile, team constitution, protocols,
  and roster.
- Use `shared/` for durable shared docs and `SHAREDMEMORY.md` for compact
  broadcast facts.
