...
3. Use **specific model names**, not provider names:
   ...
   - Codex: check `../config/codex_models.json` and use an actual id such as `gpt-5.4`, `gpt-5.2-codex`, `gpt-5.1-codex-max`, or `gpt-5.1-codex-mini`
   - Gemini: `gemini-2.5-pro`, `gemini-2.5-flash` (check `../config/gemini_models.json`)
4. Provider is `claude`, `codex`, or `gemini`
...
7. Bootstrap parity is automatic: the new agent gets full workspace init, current rule files (`AGENTS.md` / `CLAUDE.md` / `GEMINI.md`, including the Research Stack guidance), `shared/`, `memory_system/shared-core`, `project-vault` when present on `main`, mirrored skill symlinks, and context-hygiene files
8. Delegated-task status is part of parity: new agents should expose `ACCEPTED`, `BLOCKED`, and `DONE`, but Telegram visibility must follow the visible owner chat rather than every touched bot. `main -> specialist` statuses belong in the main chat; `agent A -> agent B` statuses belong in agent A's user-facing chat; direct-chat work should not emit technical lifecycle messages for that same agent's own replies; upward summaries like `agent A -> main` should stay silent unless explicitly requested. The final substantive answer still comes from `main` when the user came through `main`. User-facing Telegram wording should be Russian and human-readable (for example `Задача от главного агента получена` / `Задача от главного агента исполнена`), and raw system ids like `main` should not be shown to the user
...
**Important architecture note:** bootstrap parity does NOT create one shared live context.
Each agent still keeps its own sessions, memory, and open loops.
...
`agents/<name>/workspace/JOIN_NOTIFICATION.md` directly.
