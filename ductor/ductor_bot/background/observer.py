...
class BackgroundObserver:
    ...
    async def shutdown(self, *, graceful_timeout: float = 0.0) -> None:
    ...
    async def _drain_active(self, timeout_seconds: float) -> None:
        """Wait briefly for active background tasks to finish on their own."""
            ...
            with contextlib.suppress(Exception):
                await self._deliver(
                    BackgroundResult(
                        ...
                        thread_id=bg_task.thread_id,
                        prompt_preview=bg_task.prompt[:60],
                        result_text=_INTERRUPTED_RESULT_TEXT,
                        status="aborted",
                        elapsed_seconds=elapsed,
                        provider=bg_task.provider,
                        model=bg_task.model,
